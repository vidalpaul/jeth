...
class Transaction {
  ...
  static validateStandardTransaction({ state, transaction }) {
    return new Promise((resolve, reject) => {
      ...
      if (!toAccount) {
        return reject(new Error(`The to field ${toBalance} does not exist`));
      }
      if (toAccount.codeHash) {
        ...
        if (gasUsed > gasLimit) {
          return reject(
            new Error(
              `The transaction needs more gas. Provided: ${gasLimit}. Needs: ${gasUsed}`
            )
          );
        }
      }
  ...
  static validateMiningRewardTransaction({ transaction }) {
    return new Promise((resolve, reject) => {
      ...
      if (value !== MINING_REWARD) {
        return reject(
          new Error(
            `The provided mining reward value ${value} is not equal the official value ${MINING_REWARD}`
          )
        );
      }
      return resolve();
